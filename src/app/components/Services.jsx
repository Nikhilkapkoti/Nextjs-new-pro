...
export default function Services({ darkMode }) {
  return (
    <main id="services" className={`${styles.servicesMain} ${darkMode ? styles.darkMode : ''}`}>
      <section className={styles.booba}>
        <h2>What I Offer</h2>
        <h1>My Services</h1>
        <h3>Discover the range of services I provide to help your business thrive.</h3>
      </section>
...
function ServiceCard({ service }) {
  ...
  return (
    <div className={styles.serviceCard}>
      {service.Image && <img src={service.Image} alt={service.title} className={styles.serviceImage} />}
      <h2>{service.title}</h2>
      <p>{isExpanded ? service.fullDescription : service.shortDescription}</p>
